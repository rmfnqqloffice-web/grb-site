...
# GROUPB 디자인 규칙 (프론트엔드 파일 편집 시에만 로드)

> 아래 일반 디자인 시스템 원칙보다 **이 GROUPB 우선 조항이 항상 우선**한다.

## GROUPB 우선 조항 (generic 원칙에 우선)

1. **기존 디자인 시스템 우선.** 이 저장소의 기존 컴포넌트·shadcn/Tailwind 컨벤션·토큰이 아래 제네릭 미학 의견보다 우선한다. 새 UI 작업 전, 있으면 `design-system-extract`로 기존 토큰·패턴을 먼저 추출해 그대로 따른다. 제네릭 취향(예: 에디토리얼-웜 룩)을 이미 통일된 앱에 강요하지 않는다.
2. **전 앱 공통 규약 준수** (`~/.claude/CLAUDE.md` §7·§8):
   - 데이터 목록 테이블: 행 `align-middle` + 페이지네이션(기본 20 · 20/50/100 드롭다운 · 하단 숫자 페이징). 레퍼런스 `articles-table.tsx`.
